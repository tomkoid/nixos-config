function __fish_poetry_9cf82bc144790825_complete_no_subcommand
    for i in (commandline -opc)
        if contains -- $i about add build cache check config debug dynamic-versioning env export help init install list lock new publish remove run search self shell show source update version
            return 1
        end
    end
    return 0
...
complete -c poetry -f -n '__fish_poetry_9cf82bc144790825_complete_no_subcommand' -a dynamic-versioning -d 'Apply the dynamic version to all relevant files and leave the changes in-place. This allows you to activate the plugin behavior on demand and inspect the result. Your configuration will be detected from pyproject.toml as normal.'
